...
**Session lifetime: 400 days idle, renewed daily while in use.** `SESSION_TTL_SECONDS` (`src/lib/session-ttl.ts`) sets both the JWT `exp` and the cookie `Max-Age`. It is 400 days because that is the longest browsers keep a cookie; anything longer is clamped silently. `GET /api/user/profile`, which `CurrentUserProvider` calls on every app load, runs `renewSessionIfStale` (`src/lib/session-renewal.ts`): a token more than a day old is re-signed with claims **read from the DB row**, so an active user is never signed out and a role change reaches the middleware's `/admin` gate within a day. Renewal happens there and not in middleware because the edge has no DB and could only copy stale claims forward. The Play reviewer stays capped at `role: 'USER'` on renewal. **Tokens cannot be revoked server-side**: sign-out only removes the cookie from that browser, a copied token stays valid until its `exp`, and rotating `JWT_SECRET` signs out everyone.
